# TradeCheck AI

**Amazon vs Alibaba Product Profitability Analyzer**

---

## 📌 Descripción

TradeCheck AI es un proyecto académico que tiene como objetivo ayudar a emprendedores a evaluar la rentabilidad potencial de productos antes de importarlos, mediante la comparación de información entre Amazon y Alibaba utilizando análisis de datos e inteligencia artificial.

---

## 🎓 Contexto Académico

Proyecto desarrollado para la asignatura **Desarrollo de Sistemas de la Información**  
Universidad Central del Ecuador

---

## 🎯 Objetivo General

Desarrollar una plataforma que permita comparar productos entre Amazon y Alibaba para estimar su rentabilidad antes de la importación, reduciendo el riesgo en la toma de decisiones comerciales.

---

## 🧩 Alcance Inicial

En una primera etapa, el proyecto se enfoca en la definición del problema, análisis del contexto y preparación conceptual del sistema. Algunas funcionalidades serán simuladas en fases posteriores con el fin de validar el flujo completo antes de su implementación real.

---

## 🧪 Estado del Proyecto

- ✅ Fase 0 — Contexto y análisis del problema
- 🔄 Fase 1 — Metodología ágil y planificación (en progreso)

---

## 🛠️ Tecnologías (preliminar)

- Python  
- Streamlit  
- MongoDB Atlas  
- Inteligencia Artificial  
- Git y GitHub  

> *Las tecnologías se detallarán y justificarán en fases posteriores del proyecto.*

---

## 📁 Documentación

La documentación académica del proyecto se encuentra organizada por fases dentro del repositorio.

---

## 📌 Nota

Este proyecto se desarrolla siguiendo una metodología ágil incremental, comenzando con un análisis conceptual y evolucionando hacia un MVP funcional y una implementación real.
